...
temp <- temp |>
  mutate(
    actual_result_home = -(as.numeric(str_extract(result_home, "\\d+(?=-)")) - 
                             as.numeric(str_extract(result_home, "(?<=-)\\d+"))),
    actual_result_away = -(as.numeric(str_extract(result_away, "\\d+(?=-)")) - 
                             as.numeric(str_extract(result_away, "(?<=-)\\d+")))
  )
 
# factors
temp <- temp |>
  mutate(season = parse_factor(season),
         home_team = parse_factor(home_team),
         away_team = parse_factor(away_team),
         week = parse_factor(as.character(week)),
         game_time = parse_factor(game_time),
         day = parse_factor(day),
         ou_result = parse_factor(ou_result),
         vs_line_home = parse_factor(vs_line_home),
         vs_line_away = parse_factor(vs_line_away),
         )
